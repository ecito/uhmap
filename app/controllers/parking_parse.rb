...
if (File.new(File.dirname(__FILE__) + "/parking_output.json").ctime + (5 * 60) < Time.new)
	puts "updating parking info from UH"
	output_file = File.open(File.dirname(__FILE__) + "/parking_output.json", "w")
	puts output_file.syswrite(get_parking)
	output_file.close
